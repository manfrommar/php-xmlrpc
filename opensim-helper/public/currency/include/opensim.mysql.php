...
function  opensim_get_avatar_session($uuid, &$db=null)
{
	...
	if ($OpenSimVersion==OPENSIM_V07) {
		$sql = "SELECT RegionID,SessionID,SecureSessionID FROM Presence WHERE UserID='".$uuid."' ORDER BY LastSeen DESC ";
		$db->query($sql);
...
function  opensim_get_regions_infos($condition='', &$db=null)
{
	...
	$items = ' regions.uuid,regionName,locX,locY,sizeX,sizeY,serverIP,serverURI,serverHttpPort,owner_uuid,simulator.estate_map.EstateID,EstateOwner,EstateName,';
 	$join1 = ' FROM regions LEFT JOIN simulator.estate_map ON RegionID=regions.uuid ';
 	$join2 = ' LEFT JOIN simulator.estate_settings ON simulator.estate_map.EstateID=simulator.estate_settings.EstateID ';
...
function  opensim_set_region_estate($region, $estate, $owner, &$db=null)
{
	...
	$db->query("UPDATE simulator.estate_map SET EstateID='$estate_id' WHERE RegionID='$region'");
...
{	
	...
	$db->query("SELECT EstateID FROM simulator.estate_settings WHERE EstateName='$estate' AND EstateOwner='$owner'");
	if ($db->Errno==0) {
    ...
    $db->query("INSERT INTO simulator.estate_settings ($insert_columns) VALUES ($insert_values)");
...
function  opensim_get_estates_infos(&$db=null)
{
	...
	$db->query('SELECT EstateID,EstateOwner,EstateName FROM simulator.estate_settings ORDER BY EstateID');
	if ($db->Errno==0) {
...
function  opensim_get_estate_info($region, &$db=null)
{
	...
	if ($db->exist_table('UserAccounts')) {
		$rqdt = 'PrincipalID,FirstName,LastName,simulator.estate_settings.EstateID,EstateOwner,EstateName';
		$tbls = 'UserAccounts,simulator.estate_map,simulator.estate_settings';
		$cndn = "RegionID='$region' AND simulator.estate_map.EstateID=simulator.estate_settings.EstateID AND EstateOwner=PrincipalID";
	}
	else if ($db->exist_table('users')) {
		$rqdt = 'UUID,username,lastname,simulator.estate_settings.EstateID,EstateOwner,EstateName';
		$tbls = 'users,simulator.estate_map,simulator.estate_settings';
		$cndn = "RegionID='$region' AND simulator.estate_map.EstateID=simulator.estate_settings.EstateID AND EstateOwner=UUID";
	}
	else {
		return null;
	}
...
function  opensim_set_region_estateid($region, $estateid, &$db=null)
{
	...
	$db->query("SELECT EstateID FROM simulator.estate_settings WHERE EstateID='$estateid'");
	list($esid) = $db->next_record();
	if (intval($esid)==0) return;

	$db->query("SELECT EstateID FROM simulator.estate_map WHERE RegionID='$region'");
	list($esid) = $db->next_record();

	if (intval($esid)!=$estateid) {
		$db->query("UPDATE simulator.estate_map SET EstateID='$estateid' WHERE RegionID='$region'");
	}
	else if (intval($esid)==0) {
		$db->query("INSERT INTO simulator.estate_map (RegionID,EstateID) VALUES ('$region','$estateid')");
	}
}
...
function  opensim_set_estate_owner($region, $owner, &$db=null)
{
	...
	$db->query("UPDATE simulator.estate_settings,simulator.estate_map SET EstateOwner='$owner' WHERE simulator.estate_settings.EstateID=simulator.estate_map.EstateID AND RegionID='$region'");
	$errno = $db->Errno;
...
function  opensim_del_estate($id, &$db=null)
{
	...
	$db->query("DELETE from simulator.estate_settings WHERE EstateID=$id");
}
...
function  opensim_update_estate($id, $name, $owner, &$db=null)
{
	...
	if ($name) {
		$db->query("UPDATE simulator.estate_settings SET EstateName='$name' WHERE EstateID=$id");
	}
		
	$uuid = opensim_get_avatar_uuid($owner);
	if ($uuid) {
		$db->query("UPDATE simulator.estate_settings SET EstateOwner='$uuid' WHERE EstateID=$id");
	}
}
