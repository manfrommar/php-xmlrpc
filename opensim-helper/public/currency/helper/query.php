...
function dir_popular_query($method_name, $params, $app_data)
{
	...
	if ($DbLink->Errno==0) {
		...
		while (($row = $DbLink->next_record())) {
			...
			$data[] = array("parcel_id"=>$row["infoUUID"], "name"=>$name, "dwell"=>$row["dwell"]);
		}
		$response_xml = xmlrpc_encode(array('success'=>True, 'errorMessage'=>"", 'data'=>$data));
	}
	else {				 
...
function dir_land_query($method_name, $params, $app_data)
{
	...
	if ($type != 4294967295){ 	//Include all types of land?
		...
		if (($type & 26)==2) { 	// Auction (from SearchTypeFlags enum)
			$response_xml = xmlrpc_encode(array('success'=>False, 'errorMessage'=>"No auctions listed"));
			echo $response_xml;
			return;
		}
...
//	$pieces   	= explode(" ", $text);
	...
	$itm = "owneruuid,name,eventid,creatoruuid,category,description,dateutc,duration,".
...
function event_info_query($method_name, $params, $app_data)
{
	...
	$itm = "owneruuid,name,eventid,creatoruuid,category,description,dateutc,duration,covercharge,".
